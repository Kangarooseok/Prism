...
|----------|
|![압축 CCTV 관리](https://github.com/user-attachments/assets/595ff1d7-f9e0-4474-856c-c1779393847b)|
...
PRISM 프로젝트에서 저는 기획부터 데이터 모델링, 백엔드 CCTV API, 프런트엔드, 컨테이너 배포까지 엔드투엔드로 책임졌습니다. 단일 프런트 도메인에서 여러 백엔드를 통합해야 했고, Ingress가 타 네임스페이스 Service를 직접 참조하지 못하는 제약을 실제로 겪었습니다. 이를 해결하기 위해 프런트 네임스페이스에 백엔드별 별칭 Service를 두고, 서비스 이름을 기준으로 경로를 나누는 라우팅 규칙으로 호출 체계를 표준화했습니다. 이 과정에서 네임스페이스 경계·경로 규칙·프록시 계층을 일관되게 설계하면 단일 도메인 통합이 안정화되고, 서비스 증감에도 유연하게 대응하며, 서비스별 접근제어·관측·트래픽 및 배포 전략을 독립적으로 운용할 수 있음을 배웠습니다.
